...
prs.init(extra_options="-constant_seed")
...
def sample_folding(sequence,
    ...
    prs.pose_from_sequence(sequence)

    # 2. create a full atom reference copy of the pose
    test_pose = prs.Pose()
    ...
    to_fullatom = prs.SwitchResidueTypeSetMover('fa_standard')
    ...
    movemap = prs.MoveMap()
    movemap.set_bb(True)
    # minimizing the centroid chi angles (the side chain centroid atoms) is
    #    almost always USELESS since this compression is performed for speed,
    #    not accuracy and clashes usually occur when converting to full atom
    ...
    insert_long_frag = rosetta.protocols.moves.RepeatMover(long_frag_mover, long_inserts)
    insert_short_frag = rosetta.protocols.moves.RepeatMover(short_frag_mover, short_inserts)
    ...
    # for high-resolution, full atom, poses necessary for scoring final output
    ...
    folding_mover = rosetta.protocols.moves.SequenceMover()
    ...
    folding = rosetta.protocols.moves.RepeatMover(trial, cycles)

    # 12. create a (Py)JobDistributor
    jd = prs.PyJobDistributor(job_output, jobs, scorefxn_high)
    ...
    while not jd.job_complete:
        ...
        # -convert the decoy to full atom
        # the side-chain conformations will all be default,
        #    normally, the decoys would NOT be converted to full atom before
        #    writing them to PDB (since a large number of trajectories would
        #    be considered and their full atom score are unnecessary)
        # here the full atom mode is reproduced to make the output easier to
        #    understand and manipulate, PyRosetta can load in PDB files of
        #    centroid structures, however you must convert to full atom for
...
def main(options):
    ...
    if fasta_filename:  # defaults to off, empty string
        ...
        prs.pose_from_file(pose, pdb_filename)
        sequence = pose.sequence()
    # Checks for the sequence in a fasta, then direct, and finally from a PDB file.
    # If no PDB file is given, it will load the default.
